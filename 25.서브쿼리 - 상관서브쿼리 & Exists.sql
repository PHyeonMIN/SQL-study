...
where exists (select order_id from nw.order_items x where a.order_id = x.order_id and x.amount > 100);	--order_id가 1이되어도 상관X true인지만 체크함
...
select * from nw.customers a 
where exists (select 1 from nw.orders x 
						where x.customer_id = a.customer_id and x.order_date >= to_date('19970101', 'yyyymmdd'));

--1997년 이후에 단 한건도 주문하지 않은 고객 정보
select * from nw.customers a 
where not exists (select 1 from nw.orders x 
							where x.customer_id = a.customer_id and x.order_date >= to_date('19970101', 'yyyymmdd'));
